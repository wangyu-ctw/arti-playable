 ...
 * 首次运行会把原片备份到同目录 .orig/ 下，之后每次都【从原片】重编码——
...
const origDir = join(dir, '.orig'), orig = join(origDir, name), stateFile = join(origDir, name + '.json');
...
console.log(`不满意画质回退：cp '${orig}' '${target}'（并删 ${basename(stateFile)} 重置档位）`);
